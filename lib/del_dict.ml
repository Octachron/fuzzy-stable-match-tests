...
let pure words = String_map.of_seq @@ Seq.mapi (fun i w -> prefix w, Int_set.singleton i) words

let add_word_indices dict (dword,indices) =
  let new_set =
    match String_map.find_opt dword dict with
    | None -> indices
    | Some s -> Int_set.union indices s
  in
  String_map.add dword new_set dict

let make del_dict =
  String_map.fold (fun wd index_set new_dict ->
      let s = Seq.map (fun dw -> dw, index_set) @@ one_deletions wd in
      Seq.fold_left add_word_indices new_dict s
    ) del_dict String_map.empty
...
type search_data = {
  words: string array;
  del_dictionaries: Int_set.t String_map.t Dynarray.t
}
